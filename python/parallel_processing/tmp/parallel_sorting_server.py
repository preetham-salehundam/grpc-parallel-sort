...
class ParallelSortingServicer(route_guide_pb2_grpc.RouteGuideServicer):
    def __init__(self):
        self.busy = False
        self.internal_array =[]
        self.stub = None
    ...
    def start_connection(self, _ , context):
        ...
        if(not self.busy):
            ...
            return route_guide_pb2.Status(status=self.busy)
        else:
            logging.debug("#### server busy #####")
            return route_guide_pb2.Status(status=False)
    ...
    def connect_to(self, machine_addr, context):
        ...
        self.stub = route_guide_pb2_grpc.RouteGuideStub(channel)
        return route_guide_pb2.Status(status=self.stub is not None)
    ...
    def  end_connection(self, _, context):
        ...
        return route_guide_pb2.Status(status=not self.busy)

    def compare(self, data, context):
        result= data.a - data.b
        return route_guide_pb2.Result(result=result)
    ...
    def forward_data_to_peer(self, data, context):
        ...
        data = [route_guide_pb2.Data(item=x.item) for x in data]
    ...
    def fetch_result_from_peer(self, _ , context):
        ...
        result = [route_guide_pb2.Data(item=x.item) for x in result]
        for item in result:
            yield item
    ...
    def process_data(self, data_a, context):
        ...
        while(not max_A < min_B):
            ...
            data = route_guide_pb2.Tuple(a=max_A, b=min_B)
        ...
        for item in self.internal_array:
            yield route_guide_pb2.Data(item=item)  
        #pass
    ...
    def get_max(self,_, context):
        ...
        return route_guide_pb2.Result(result = max(self.internal_array))
        #pass
    ...
    def swap(self, data,  context):
        ...
        return route_guide_pb2.Result(result=data.b)
        #pass
    ...
    def remote_sort(self, _ , context):
        ...
        data = [route_guide_pb2.Data(item=x) for x in self.internal_array]
        for item in data:
            yield item
        #pass
    ...
    def get_partial_sorted_data(self, _ , context):
        ...
        data = [route_guide_pb2.Data(item=x) for x in quicksort(self.internal_array)]
        for item in data:
            yield item
